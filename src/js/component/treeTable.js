 ...
 * 树形表格
 */
layui.define(['jquery', 'storage','conf'], function (exports) {
    let MOD_NAME = 'treeTable',
        o = layui.jquery,
        tree = function () { };
    ...
    tree.prototype.template = function (e) {
        ...
        o.each(t.data(e, e.data), function (idx, item) {
            ...
            o.each(e.cols, function (index, obj) {
                ...
                // 指定列加入开启、关闭小图标
                let icon = (obj.key == e.icon_key && !item.is_end) ? '<i class="' + (t.cache(e, item[e.primary_key]) ? e.icon.open : e.icon.close) + '"></i>' : '<span></span>';
                // 指定列小图标按照层级向后位移
                let left = (obj.key == e.icon_key ? level[item[e.primary_key]] * e.icon.left + 'px' : '');
            ...
            let box = is_table ? o(is_checked ? checked : checkbox).wrap('<td style="width:28px;">').parent().prop('outerHTML') : '';
            tbody += '<tr class="' + hide_class + '" data-id="' + item[e.primary_key] + '" data-pid="' + item[e.parent_key] + '">' + box + tr + '</tr>';
        });
        // 处理表树和树的赋值模板
        let table = is_table ? '<thead><tr data-id="' + e.top_value + '">' + thead + '</tr></thead><tbody>' + tbody + '</tbody>' : tbody.replace(/<tr/g, '<ul').replace(/tr>/g, 'ul>').replace(/<td/g, '<li').replace(/td>/g, 'li>');
        // 确认点击图标或点击列触发展开关闭
        let click_btn = e.is_click_icon ? '[data-down] i:not(.layui-icon-ok)' : '[data-down]';
        // 模板渲染并处理点击展开收起等功能
        o(e.elem).html(table).off('click', click_btn).on('click', click_btn, function () {
            let tr = o(this).parents('[data-id]'),
            ...
            let is_checked = o(this).toggleClass('layui-form-checked').hasClass('layui-form-checked'),
    ...
    tree.prototype.parents_checkbox = function (e, pid) {
        let po = o(e.elem).find('[data-pid=' + pid + ']'),
    ...
    tree.prototype.render = function (e) {
        ...
        if (e.url) {
            let header = {}
            header[layui.conf.tokenName] = layui.storage.getToken();
            o.get({
                url: e.url,
                headers: header,
    ...
    tree.prototype.checked = function (e) {
        ...
        o(e.elem).find('.cbx.layui-form-checked').each(function () {
            let id = o(this).parents('[data-id]').data('id');
            ids.push(id);
        })
        return ids;
    };
    ...
    // 设置已经选
    tree.prototype.setChecked = function (e, id) {
        let ele = o(e.elem).find('[data-id=' + id + ']')
        ele.find('td').children().addClass('layui-form-checked')
    };
    ...
    tree.prototype.cache = function (e, val, option) {
        ...
        let t = this,
            ...
            index = o.inArray(val1, cache);
        if (option === undefined) {
            return index == -1 ? false : val1
        }
        if (option && index == -1) {
            cache.push(val1)
        }
